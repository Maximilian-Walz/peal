...
# required", "model: opus", "priority: high", "owner: human" and "<field>: <value>" for the
...
# needs, size, plan, model, priority, owner and the project's fields, as "<key>: <value>".
_peal_issues_managed() {
  local key
  case $1 in *:*) ;; *) return 1 ;; esac
  key=$(printf '%s' "${1%%:*}" | tr '[:upper:]' '[:lower:]' | sed 's/^ *//; s/ *$//')
  case ",needs,size,plan,model,priority,owner,$PEAL_FIELDS," in *",$key,"*) return 0 ;; esac
  return 1
}
