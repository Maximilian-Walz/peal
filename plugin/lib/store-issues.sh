...
# "release-note: none" and "<field>: <value>" for the project's own fields.
#
...
peal_store_list() {
  ...
  if _peal_issues_scan "$tmp" && _peal_issues_prs >"$tmp/prs"; then
    ...
    cut -f1-13,16,17 "$tmp/scan" >"$tmp/tasks"
...
# needs, size, plan, model, priority, owner, breaking, release-note and the project's
# fields, as "<key>: <value>".
_peal_issues_managed() {
  local key
  case $1 in *:*) ;; *) return 1 ;; esac
  key=$(printf '%s' "${1%%:*}" | tr '[:upper:]' '[:lower:]' | sed 's/^ *//; s/ *$//')
  case ",needs,size,plan,model,priority,owner,breaking,release-note,$PEAL_FIELDS," in *",$key,"*) return 0 ;; esac
  return 1
}
