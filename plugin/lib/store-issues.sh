...
# required", "model: opus" and "<field>: <value>" for the project's own fields.
#
...
# needs, size, plan, model and the project's fields, as "<key>: <value>".
_peal_issues_managed() {
  local key
  case $1 in *:*) ;; *) return 1 ;; esac
  key=$(printf '%s' "${1%%:*}" | tr '[:upper:]' '[:lower:]' | sed 's/^ *//; s/ *$//')
  case ",needs,size,plan,model,$PEAL_FIELDS," in *",$key,"*) return 0 ;; esac
  return 1
}
