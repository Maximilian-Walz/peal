...
# (lib/store.sh):
#
#   id  state  detail  slug  title  milestone  depends  part-of  size  plan  needs  path  ref  pr  url  priority  owner
#
...
END {
  ...
  for (i = 1; i <= n; i++) {
    ...
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", id, state, detail,
      ...
      path[id], cref[id], cpr[id], url[id], prio[id], owner[id]
  }
}
